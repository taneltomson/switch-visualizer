...
\huge Võrgukommutaatorite visualiseerimine
...
võrguseade, mis ühendab seadmeid arvutivõrku.
Näiteks kontorites, kus on reeglina hulgaliselt võrguseadmeid (arvutid, printerid jne), kasutatakse
kommutaatoreid, et suur arv seadmeid saaksid arvutivõrguga ühenduda.
...
On protokolle, mille abil saab võrgukommutaatoritelt haldusinfot küsida.
Kommutaatorilt infot küsides on tihti vastuseks saadud andmete hulk suur ja vajab lisatöötlust.
Ülevaate saamine, kuidas kommutaatorid üksteisega ühendatud on, võib muutuda tülikaks ja
aeganõudvaks.
Töö autor kirjutas programmi, mis abistab võrgukommutaatorite haldamist, visualiseerides
kommutaatorite paiknemist üksteise suhtes.
Fookus on just kommutaatorite endi paiknemise visualiseerimisel, jättes lisainformatsiooni (muud
ühendatud seadmed) tagaplaanile.

Järnevalt on toodud töö struktuur.
Peatükis "\nameref{reqs}", kirjeldatakse rakenduse jooksutamiseks vajalikud tingimused
ning enne rakenduse arendamist paika pandud nõuded.
Peatükis "\nameref{similar}" tuuakse ülevaade sarnast funktsionaalsust pakkuvatest
rakendustest.
"\nameref{usedTech}" kirjeldab tehnoloogiaid, teeke ja rakendusi, mida programm kasutab või mida
kasutati arendusprotsessis.
Peatükis "\nameref{description}" tehakse detailne ja tehniline ülevaade rakenduse arhitektuurist.
Samuti põhjendatakse tehtud otsuseid ning näidatakse rakenduse nõuete täitmist.
...
\TODO{sysName kasutatakse id-na -- kas tahan rääkida sellest, et miks? indeksid erinevate mib-de
vahel erinevad}
...
\TODO{Mis andmed kuvatakse}
Hüpikvihjete implementatsioon on samuti failis \texttt{src/web/code.js}.
\TODO{Hüpikaknast näidispilt, kui koodis kõik asjad sinna lisatud on}
...
\TODO{puudus graafi servade labelitega}
